  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE))
    {
      ...
      if (!Power_Status) _delay_ms(2000); // Cold start
      if (AXIS_X_IS_PRESENT) STEPPERS_DISABLE_ALL_AXIS_PORT &= ~STEPPERS_DISABLE_AXIS_X_MASK;
      if (AXIS_Y_IS_PRESENT) STEPPERS_DISABLE_ALL_AXIS_PORT &= ~STEPPERS_DISABLE_AXIS_Y_MASK;
      if (AXIS_Z_IS_PRESENT) STEPPERS_DISABLE_ALL_AXIS_PORT &= ~STEPPERS_DISABLE_AXIS_Z_MASK;
      if (AXIS_A_IS_PRESENT) STEPPERS_DISABLE_ALL_AXIS_PORT &= ~STEPPERS_DISABLE_AXIS_A_MASK;
    }
  else
    {
      STEPPERS_DISABLE_PORT &= ~STEPPERS_DISABLE_MASK;
...
void st_go_idle()
{
  ...
  if (sys.state & (STATE_ALARM)) pin_state = true; // Override. Disable steppers.
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { pin_state = !pin_state; } // Apply pin invert.
    ...
    if (!pin_state) Power_Status = false; // Next power ON will be "cold"
}
